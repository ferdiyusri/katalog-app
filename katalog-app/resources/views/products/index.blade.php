...
.filter-tab {
    ...
    color: var(--gray); font-size: 0.85rem; font-weight: 600; letter-spacing: 0.5px;
    text-transform: uppercase; padding: 0.6rem 1.5rem; border-radius: 8px;
    cursor: pointer; transition: all 0.3s ease; text-decoration: none;
}
.filter-tab:hover { border-color: var(--accent); color: var(--accent); }
.filter-tab.active { background: var(--primary); border-color: var(--primary); color: var(--white); }
.filter-tab-sep { width: 1px; height: 22px; background: rgba(0,0,0,0.1); align-self: center; flex-shrink: 0; margin: 0 0.15rem; }
.search-box { position: relative; }
.search-box input { border: 1.5px solid rgba(0,0,0,0.1); border-radius: 8px; padding: 0.6rem 1rem 0.6rem 2.8rem; font-size: 0.9rem; font-family: 'DM Sans', sans-serif; width: 280px; transition: all 0.3s ease; background: var(--light); }
...
.product-card { background: var(--white); border: none; border-radius: 12px; overflow: hidden; transition: all 0.4s ease; box-shadow: 0 2px 15px rgba(0,0,0,0.04); height: 100%; position: relative; }
...
.btn-daftar-harga { display: inline-flex; align-items: center; gap: 0.4rem; background: transparent; border: 1.5px solid var(--accent); color: var(--accent); font-size: 0.78rem; font-weight: 600; letter-spacing: 0.5px; padding: 0.4rem 0.9rem; cursor: pointer; transition: all 0.3s ease; border-radius: 6px; white-space: nowrap; }
...
.pagination-custom .page-link { border: 1.5px solid rgba(0,0,0,0.1); border-radius: 6px; color: var(--primary); font-weight: 600; font-size: 0.88rem; padding: 0.6rem 1rem; transition: all 0.3s ease; }
...
<section class="page-header">
    ...
    <div class="container position-relative">
        <div class="page-header-label">Koleksi Lengkap</div>
        <h1 class="page-header-title">Semua Produk</h1>
        <p class="page-header-desc">Jelajahi seluruh koleksi produk desain interior dan konstruksi berkualitas tinggi dari HIKO STUDIO.</p>
    </div>
</section>
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div class="filter-tabs">
                <a href="{{ route('products.index') }}"
                    class="filter-tab {{ !$kategori ? 'active' : '' }}">Semua</a>
                @foreach($kelompokAll as $grp)
                    <span class="filter-tab-sep"></span>
                    <a href="{{ route('products.index', ['kategori' => $grp->slug]) }}"
                        class="filter-tab {{ $kategori == $grp->slug ? 'active' : '' }}">
                        {{ $grp->name }}
                    </a>
                @endforeach
            </div>
            <div class="d-flex align-items-center gap-3">
                <span class="product-count"><strong>{{ $products->total() }}</strong> produk ditemukan</span>
        ...
        <div class="row g-4 fade-up" id="productGrid">
            ...
            <div class="col-md-6 col-lg-4 product-item" data-name="{{ strtolower($product->name) }}">
                <div class="product-card">
                    ...
                    <div class="product-card-body">
                        <div class="product-card-category">{{ ucwords(str_replace('-', ' ', $product->category)) }}</div>
            ...
            <div class="col-12">
                <div class="empty-state">
                    ...
                    <p>Produk belum tersedia. Silakan tambahkan melalui panel admin.</p>
                </div>
            </div>
            @endforelse
        </div>
...
<div class="modal fade" id="modalDaftarHarga" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:14px; border:none; box-shadow:0 30px 80px rgba(0,0,0,0.2); overflow:hidden;">
